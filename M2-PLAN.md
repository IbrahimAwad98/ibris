...
Write pipeline, in order: save to `.<name>.ibris-tmp` in the target
directory → close the PDFium document (releases the original file) →
`rename` over the target (same-volume atomic on NTFS) → reopen → remap the
tab's docId → `savedCursor = cursor`, delete sidecar. A failure before the
rename leaves the original untouched; the temp file is cleaned up on next
...
**Existing annotations in opened documents** are read and displayed
(PDFium enumerates them), can be deleted, but are not editable in M2 —
editing arbitrary foreign appearance streams is a rabbit hole. Deletion is a
normal `remove-annotation` command (the removed value captured for undo is
our parsed form; its /AP is regenerated by us if undo re-adds it — a
foreign annotation that survives delete+undo may therefore change visual
detail; accepted and logged).
